...
const ProductList = () => {
  ...
  return (
    <div>
      <button className='btn btn-dark m-auto d-block mt-2' onClick={logout}>Logout</button>
      <div className='d-flex align-items-center justify-content-around mt-2'>
        ...
        <button className='btn btn-dark' onClick={(e) => setOrder('asc')}>Low to High</button>
        <button className='btn btn-dark' onClick={(e) => setOrder('desc')}>High to Low</button>
      </div>
      <div className='d-flex flex-wrap justify-content-around'>
        {data.length > 0 ? data.map((e, index) => (
          <Card style={{ width: '18rem' }} className='mb-2 mt-4 text-center' key={e.id}>
            {index + 1}
            <Link
              style={{ textDecoration: "none" }}
              to={`/productdetail/${e.id}`}
            >
            <Card.Img variant="top" src={e.image} height={250} width={250} className='object-fit-contain' />
            </Link>
            <Card.Body>
